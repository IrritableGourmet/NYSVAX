...
class CountyHandler{
	...
	initialize(){
		...
		CountiesIn.forEach((el)=>{
			try{
				if(el){
					bIndex = this.Counties.findIndex(b=>b.name === el[0]);
					if(bIndex > -1)
						this.Counties[bIndex].addZip(el[1]);
					else{
						let tempCounty = new County(el[0]);
						tempCounty.addZip(el[1]);
						this.Counties.push(tempCounty);
					}
				}
			}catch(e){ console.log(e); }	
		});
	...
	lookup(){
		...
		if(zip && County.zipRegex.test(zip)){
			this.output.innerHTML = zip + " is in " + this.findCounty(zip) + " county";
		}else{
